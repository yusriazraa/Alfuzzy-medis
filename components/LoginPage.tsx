...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (username === 'santri' || username === 'orangtua' || username === 'admin') {
      onLogin(username);
    } else {
      setError('Username tidak valid. Coba: santri, orangtua, atau admin.');
    }
  };
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Gunakan "santri", "orangtua", atau "admin" sebagai username untuk demo.
          </p>
        </div>
